...
class BinarySearchTree
  ...
  def insert(score, movie_title, depth = 0)
    if !@root
      @root = Node.new(score, movie_title)
      @root.depth = 0
    else
      parent_node = find_parent(score)
      if parent_node.score > score
        parent_node.left = Node.new(score, movie_title)
        new_node = parent_node.left
      else
        parent_node.right = Node.new(score, movie_title)
  ...
  def sort(movie_titles_asc = [], node = @root)
    movie_titles_asc << node.data if !movie_titles_asc.include?(node.data)
    index = movie_titles_asc.find_index(node.data)
    movie_titles_asc.insert(index, node.left.data) if node.left
    movie_titles_asc.insert(index+2,node.right.data) if node.right
    sort(movie_titles_asc, node.left) if node.left
    sort(movie_titles_asc, node.right) if node.right
    movie_titles_asc.compact
  end
